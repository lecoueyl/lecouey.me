  ...
  methods: {
    onMouseMove(event) {
      ...
      if (this.linkElement.includes(event.target.localName) && !this.isHoverLink) {
        ...
        const targetViewportOffset = event.target.getBoundingClientRect();
        gsap.to(this.$refs.cursor, {
          duration: cursor.duration.move,
          x: targetViewportOffset.left,
          y: targetViewportOffset.top + (event.target.offsetHeight / 2),
        });
        gsap.to(this.$refs.dot, {
          duration: cursor.duration.scale,
          ease: 'back.out(3)',
          height: event.target.offsetHeight + 10,
          width: event.target.offsetWidth + 30,
        });
        // this.onMouseEnter();
      }
